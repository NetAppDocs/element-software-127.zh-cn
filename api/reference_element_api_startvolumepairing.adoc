...
[role="lead"]
您可以使用 `StartVolumePairing`方法从用于与其他卷配对的卷创建编码密钥。此方法创建的密钥将在方法中 `CompleteVolumePairing`用于建立卷配对。
...
* `Async`：写入操作在本地完成后得到确认。集群不会等待写入复制到目标集群。（如果未指定模式参数，则为默认值。）
* `Sync`：当数据存储在本地和远程集群上时，源将确认写入。
...
卷 PairingKey
 a| 
由API方法使用的字符串xref:reference_element_api_completevolumepairing.adoc[CompleteVolumePairing]。
 a| 
string
