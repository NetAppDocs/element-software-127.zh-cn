...
* 使用新的NetApp HCC角色link:task_mnode_create_netapp_hcc_role_vcenter.html#add-an-asset-to-the-management-node["添加资产"]访问管理节点。
...
[NOTE]
====
*** 从适用于vCenter Server 5.0的NetApp Element插件开始，要使用 https://docs.netapp.com/us-en/vcp/vcp_concept_linkedmode.html["vCenter 链接模式"^]，您需要从单独的管理节点为管理NetApp SolidFire存储集群的每个vCenter Server注册Element插件(建议)。
*** 使用适用于vCenter Server 4.10及更早版本的NetApp Element插件管理其他vCenter Server中的集群资源 https://docs.netapp.com/us-en/vcp/vcp_concept_linkedmode.html["vCenter 链接模式"^]仅限于本地存储集群。
...
如果您需要在安装后手动添加新资产，请使用您在中创建的新HCC用户帐户<<在 vCenter 中设置新用户帐户>>。有关详细信息，请参见 link:task_mnode_add_assets.html["向管理节点添加控制器资产"]。
