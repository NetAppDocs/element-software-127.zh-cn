...
.. 为要添加的每个默认时间服务器添加一行。默认时间服务器必须与您将在 <<set-up-the-management-node,后续步骤>> 中使用的存储集群上使用的 NTP 服务器相同。
+
[listing]
----
vi /etc/ntp.conf
...
*** *--storage_mvip (-sm) [MVIP地址]*：运行 Element 软件的存储集群的管理虚拟 IP 地址 (MVIP)。使用与 link:task_mnode_recover.html#configure-time-sync["NTP 服务器配置"] 期间所用的相同存储集群配置管理节点。
