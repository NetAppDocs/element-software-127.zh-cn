...
IMPORTANT: 对于运行 Element 软件 12.0 版的 H610S 存储节点，在升级到存储固件包 2.27 之前，应应用 D-patch SUST-909 。在升级之前，请联系 NetApp 支持部门获取 D-patch 。请参阅。 link:http://docs.netapp.com/us-en/hci/docs/rn_storage_firmware_2.27.html["存储固件包2.27发行说明"^]


IMPORTANT: 在升级存储节点上的固件之前，您必须升级到最新的管理服务包。如果要将Element软件更新到版本12.2或更高版本、则需要使用管理服务2.14.60或更高版本才能继续。

* * 集群运行状况 * ：您已运行运行运行状况检查。请参阅。 link:task_hcc_upgrade_element_prechecks.html["在升级存储之前，请运行 Element 存储运行状况检查"]
* *已更新H610S节点的基板管理控制器(BMC)*：您已升级H610S节点的BMC版本。请参阅。 link:https://docs.netapp.com/us-en/hci/docs/rn_H610S_BMC_3.84.07.html["发行说明和升级说明"^]



NOTE: 有关硬件link:../hardware/fw_storage_nodes.html["SolidFire 存储节点支持的存储固件版本"]的固件和驱动程序固件的完整列表，请参阅。
...
.步骤
. 将存储固件升级包下载到可供管理节点访问的设备；转到Element软件 https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab["下载页面"^]并下载最新的存储固件映像。
. 将存储固件升级软件包上传到管理节点：
+
